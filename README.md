...
3. If you want to use webhooks, you can create self-signed SSL certificate by executing `./gen_ssl` shell script. Before executing, run `chmod +x gen_ssl` to allow executing permission on this script
4. Rename `env.example` to `.env` (by executing `mv env.example .env`) and replace variables to your own
...
5. Run your bot: `python -O bot.py`
...
- Autogenerate new migration version: `alembic revision --autogenerate -m 'name_of_migration'`
